...
import { Calendar, Plus, Pencil, Trash2, Users, LogOut, ExternalLink, Check, X } from "lucide-react";
...
interface HolidayEntry {
  ...
  entry_type: "holiday" | "absence" | "late" | "request" | string;
  start_date: string;
...
export const HolidaysTab = ({ careGiverId, careGiverName }: Props) => {
  ...
  const [defaultType, setDefaultType] = useState<string>("holiday");
  const [confirmDeleteId, setConfirmDeleteId] = useState<string | null>(null);

  const { data: entries = [], isLoading } = useQuery({
    queryKey: ["caregiver_holidays", careGiverId],
    queryFn: async () => {
  ...
  const upsert = useMutation({
    mutationFn: async (e: Partial<HolidayEntry> & { id?: string }) => {
      if (e.id) {
        const { error } = await supabase
          .from("caregiver_holidays").update(e).eq("id", e.id);
        if (error) throw error;
      } else {
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["caregiver_holidays", careGiverId] });
      setDialogOpen(false);
      setEditing(null);
      toast.success("Saved");
    },
  ...
  const remove = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from("caregiver_holidays").delete().eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["caregiver_holidays", careGiverId] });
  ...
  const setStatus = useMutation({
    mutationFn: async ({ id, status }: { id: string; status: string }) => {
      const { error } = await supabase
        .from("caregiver_holidays").update({ status }).eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["caregiver_holidays", careGiverId] });
      ...
      <Table>
        ...
        <TableBody>
          {isLoading ? (
            ...
            <TableRow>
              <TableCell colSpan={7} className="text-center text-xs text-muted-foreground py-10 bg-muted/20">
                {emptyMsg}
              </TableCell>
            </TableRow>
          ) : (
              ...
              <TableRow key={e.id}>
                ...
                <TableCell className="text-right">
                  <div className="flex items-center justify-end gap-1">
                    ...
                    <button onClick={() => openEdit(e)} className="text-amber-600 hover:text-amber-700">
                      <Pencil className="h-3.5 w-3.5" />
                    </button>
                    <button onClick={() => setConfirmDeleteId(e.id)} className="text-destructive hover:text-destructive/80">
        ...
        <CardContent className="p-3">
          <Tabs value={activeSubTab} onValueChange={setActiveSubTab}>
            <div className="flex items-center justify-between gap-2 flex-wrap">
              <TabsList className="bg-muted/30">
                <TabsTrigger value="holidays" className="text-xs">
                  All Holidays ({filtered.holidays.length})
                </TabsTrigger>
                <TabsTrigger value="absences" className="text-xs">
                  All Absences ({filtered.absences.length})
                </TabsTrigger>
                <TabsTrigger value="lates" className="text-xs">
                  All Lates ({filtered.lates.length})
                </TabsTrigger>
                <TabsTrigger value="requests" className="text-xs">
                  Pending Requests ({filtered.requests.length})
                </TabsTrigger>
              </TabsList>
              <Button
                size="sm"
            ...
            <TabsContent value="holidays" className="mt-3">
              {renderTable(filtered.holidays, `There are no holidays for ${careGiverName} so far`)}
            </TabsContent>
            <TabsContent value="absences" className="mt-3">
              {renderTable(filtered.absences, `There are no absences for ${careGiverName} so far`)}
            </TabsContent>
            <TabsContent value="lates" className="mt-3">
              {renderTable(filtered.lates, `There are no lates for ${careGiverName} so far`)}
            </TabsContent>
            <TabsContent value="requests" className="mt-3">
              {renderTable(filtered.requests, `There are no pending requests for ${careGiverName} so far`, true)}
            </TabsContent>
          </Tabs>
        </CardContent>
      </Card>
...
const EntryForm = ({
  initial, defaultType, onSubmit,
}: {
  initial: HolidayEntry | null;
  defaultType: string;
  onSubmit: (v: Partial<HolidayEntry>) => void;
}) => {
