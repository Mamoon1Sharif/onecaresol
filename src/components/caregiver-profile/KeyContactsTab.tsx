...
const emptyDraft: Omit<KeyContact, "id" | "care_giver_id"> = {
...
export function KeyContactsTab({ careGiverId }: Props) {
  ...
  const { data: contacts = [], isLoading } = useQuery({
    queryKey: ["caregiver-key-contacts", careGiverId],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("caregiver_key_contacts")
        .select("*")
  ...
  const upsert = useMutation({
    mutationFn: async (payload: typeof emptyDraft & { id?: string }) => {
      if (payload.id) {
        ...
        const { error } = await supabase.from("caregiver_key_contacts").update(rest).eq("id", id);
        if (error) throw error;
      } else {
        const { error } = await supabase
          .from("caregiver_key_contacts")
          .insert({ ...payload, care_giver_id: careGiverId });
        if (error) throw error;
      }
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["caregiver-key-contacts"] });
  ...
  const remove = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from("caregiver_key_contacts").delete().eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["caregiver-key-contacts"] });
  ...
  const handleSave = () => {
    ...
    if (!draft.tel1?.trim()) return toast.error("Tel 1 is required (shown in the app)");
  ...
  const exportCSV = () => {
    ...
    a.href = url; a.download = `key-contacts.csv`; a.click();
    URL.revokeObjectURL(url);
  };

  const exportExcel = () => {
    // Simple HTML-table export that Excel opens natively
    const headers = ["Type", "Name", "Tel 1", "Tel 2", "Mobile", "Email", "Address 1", "Address 2", "Area", "Postcode", "NOK", "Lives With", "ICE", "Show On App", "Note"];
    const rows = filtered.map((c) =>
      `<tr>${[c.contact_type, c.name, c.tel1, c.tel2, c.mobile, c.email, c.address1, c.address2, c.area, c.postcode, c.is_nok ? "Y" : "N", c.lives_with ? "Y" : "N", c.is_ice ? "Y" : "N", c.show_on_app ? "Y" : "N", c.note].map((v) => `<td>${v ?? ""}</td>`).join("")}</tr>`
    ).join("");
    const html = `<table><thead><tr>${headers.map((h) => `<th>${h}</th>`).join("")}</tr></thead><tbody>${rows}</tbody></table>`;
    const blob = new Blob([html], { type: "application/vnd.ms-excel" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url; a.download = `key-contacts.xls`; a.click();
    URL.revokeObjectURL(url);
  };
    ...
    <Card className="border border-border shadow-sm overflow-hidden">
      ...
      <CardContent className="p-4 space-y-3">
        <p className="text-sm text-muted-foreground">
          You may add here any other contacts this person has in their life. From friends, other family members to doctors and emergency contacts.
        </p>
        <p className="text-sm text-destructive">
          Please fill out ( Tel 1 ) number, this will be shown in the app.
        </p>
        ...
        <div className="border rounded-md overflow-x-auto">
          <Table>
            <TableHeader>
              <TableRow className="bg-muted/30">
                ...
                <TableHead className="font-bold">
                  Tel 1 <span className="text-destructive">*</span>
                </TableHead>
                <TableHead className="font-bold">Tel 2</TableHead>
                <TableHead className="font-bold">Mobile</TableHead>
                <TableHead className="font-bold">Email</TableHead>
                <TableHead className="font-bold">Ad 1</TableHead>
                <TableHead className="font-bold">Ad 2</TableHead>
            ...
            <TableBody>
              {isLoading && (
                <TableRow>
                  <TableCell colSpan={17}><Skeleton className="h-12 w-full" /></TableCell>
                </TableRow>
              )}
              ...
              {filtered.map((c) => (
                <TableRow key={c.id}>
                  <TableCell>
                    <Button size="icon" variant="ghost" className="h-7 w-7 text-warning hover:text-warning" onClick={() => openEdit(c)}>
                      <Pencil className="h-3.5 w-3.5" />
                    </Button>
                  </TableCell>
                  <TableCell>
                    <Button size="icon" variant="ghost" className="h-7 w-7 text-destructive hover:text-destructive" onClick={() => setDeletingId(c.id)}>
                  ...
                  <TableCell className="text-sm">{c.address1 ?? ""}</TableCell>
                  <TableCell className="text-sm">{c.address2 ?? ""}</TableCell>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <Field label="Tel 1 *">
              <Input value={draft.tel1 ?? ""} onChange={(e) => setDraft({ ...draft, tel1: e.target.value })} />
            </Field>
            <Field label="Tel 2">
              <Input value={draft.tel2 ?? ""} onChange={(e) => setDraft({ ...draft, tel2: e.target.value })} />
            </Field>
            <Field label="Mobile">
