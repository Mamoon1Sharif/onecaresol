...
export default function ReceiverChangelog() {
  ...
  const { data: logs = [], isLoading } = useQuery({
    queryKey: ["receiver_changelog", id, date.toDateString()],
    enabled: !!id,
    queryFn: async () => {
      const { data, error } = await supabase
        .from("receiver_changelog" as any)
        .select("*")
  ...
  return (
    <div className="min-h-screen bg-muted/30">
      <ServiceUserTopBar title="Service Member Change Logs" backTo={`/carereceivers/${id}`} />

      <div className="grid grid-cols-1 lg:grid-cols-[360px_1fr] gap-4 p-4">
        <ServiceUserSidebar cr={cr} basePath="changelog" />

        <div className="space-y-3">
          <Card className="flex items-center justify-between px-3 py-2">
            <div />
            <div className="flex items-center gap-1.5">
              <Button variant="ghost" size="sm" className="h-8 w-8 p-0 text-primary" onClick={() => setDate((d) => subDays(d, 1))}>
              ...
              <Button variant="ghost" size="sm" className="h-8 w-8 p-0 text-primary" onClick={() => setDate((d) => addDays(d, 1))}>
          ...
          <Card className="overflow-hidden">
              ...
              <table className="w-full text-xs">
                ...
                <tbody>
                  {isLoading ? (
                    ...
                    <tr><td colSpan={5} className="px-4 py-8 text-center text-muted-foreground">No change logs for this date</td></tr>
                  ) : (
                    pageItems.map((l, idx) => (
                      <tr key={l.id} className={`border-t border-border ${idx % 2 === 1 ? "bg-muted/20" : ""}`}>
                        <td className="px-3 py-2 font-medium text-foreground align-top">{l.record_id}</td>
                        <td className="px-3 py-2 text-foreground align-top whitespace-nowrap">{format(parseISO(l.log_time), "yyyy-MM-dd HH:mm:ss")}</td>
                        <td className="px-3 py-2 text-foreground align-top">{l.made_by}</td>
                        <td className="px-3 py-2 align-top">
                          <div className="text-emerald-600 dark:text-emerald-400 font-medium">{l.title}</div>
                          <div className="text-foreground mt-0.5">{l.description}</div>
                        </td>
                        <td className="px-3 py-2 text-foreground align-top">{l.for_name || ""}</td>
                      </tr>
                    ))
                  )}
                </tbody>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 px-4 py-3 border-t border-border text-xs">
              <span className="text-muted-foreground">
                Showing {pageItems.length === 0 ? 0 : pageStart + 1} to {pageStart + pageItems.length} of {filtered.length} entries
              </span>
